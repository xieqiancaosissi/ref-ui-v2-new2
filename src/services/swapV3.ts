...
import { REF_UNI_SWAP_CONTRACT_ID } from "@/utils/contract";
import {
  IAddLiquidityInfo,
...
import { ftGetStorageBalanceContract } from "./ft-contract";
...
export const regularizedPoint = (point: number, fee: number) => {
  const point_delta = feeToPointDelta(fee) || 1;
...
export const create_pool = async ({
  ...
  const storageBalances = await Promise.all(
    tokenIds.map((id) => {
      return ftGetStorageBalanceContract(id);
    })
  );
  ...
  const poolStr = token_a + "<>" + token_b + "@" + fee;
...
export const get_user_storage_detail = async ({ size }: { size: number }) => {
  const user_id = window.accountId;
...
export const checkTokenNeedsStorageDeposit_v3 = async () => {
  let storageNeeded;
  const balance = await currentStorageBalanceOfV3(getAccountId());
...
export const checkTokenNeedsStorageDeposit_v3_old_version = async () => {
  let storageNeeded;
  const balance = await currentStorageBalanceOfV3_old_version(getAccountId());
