...
function DclChart({
  ...
  function get_latest_user_chart_data() {
    ...
    const list = divide_liquidities_into_bins_user({
      liquidities: nfts,
      slot_number_in_a_bin: bin_final || 0,
      tokenX: token_x_metadata,
      tokenY: token_y_metadata,
      poolDetail: pool,
    });
  ...
  function get_point_range_by_price_range() {
    ...
    // console.log(price_range_point_min, price_range_point_max, "...........");
  ...
  function getConfig(): IChartItemConfig {
    const { bin, range, colors, rangeGear } = get_default_config_for_chart(
      pool_id?.split("|")?.pop()
    ) as IChartItemConfig;
  ...
  const rangeGear: any = getConfig().rangeGear;
